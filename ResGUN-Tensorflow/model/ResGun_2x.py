...
class ResGUN(object):
    ...
    def __init__(self,img_size=32,num_layers=32,channels=256,scale=4,output_channels=3,steps=5,batch_size=10):
        ...
        x = EDSR_block(x,last=False,step_size=25,num_layers=num_layers,channels=channels,scale=0.1)
        x = EDSR_block(x,last=True,step_size=50,num_layers=num_layers,channels=channels,scale=0.1)
        # outputlayer
        '''
        for i in range(steps):
            x = conv_block(x,step_size=10,num_layers=num_layers,channels=channels)

        #output layer
        output =slim.conv2d(x,output_channels,[3,3],activation_fn=None)
    ...
    def train(self,decay_steps=1000,decay_rate=0.9,iterations=4000,save_dir="saved_models"):
        ...
        with self.sess as sess:
            ...
            for i in tqdm(range(iterations)):
                ...
                self.saver.save(sess,save_dir+"/model",global_step=1000)    
