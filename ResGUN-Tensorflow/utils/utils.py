...
def resize(x,last,step_size):
    if last:
       shape = x.get_shape().as_list()
       x = tf.image.resize_images(x,[shape[1]+step_size,shape[2]+step_size],method=tf.image.ResizeMethod.BICUBIC)
       x = slim.conv2d(x,3,[3,3],activation_fn=None)
    else:
       shape = x.get_shape().as_list()
       x = tf.image.resize_images(x,[shape[1]+step_size,shape[2]+step_size],method=tf.image.ResizeMethod.BICUBIC)
    return x
...
def EDSR_block(x,last,step_size,num_layers=16,channels=64,scale=1):
	...
	x += conv_1
	if last:
		#x = deconv2d(x,step_size,channels=3) #using the conv_trans to upsampling
                x = resize(x,last=last,step_size=step_size) # using the bicubic 
                print(x.get_shape())
	else:
		#x = deconv2d(x,step_size,channels=channels)
                x = resize(x,last=last,step_size=step_size)
	return x
