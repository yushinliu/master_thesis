...
def load_dataset(data_dir,target=['battery','PCB','BioStone']):
	...
	try:
		for items in name_set:
			img_set=[]
			count = 0
			for img in os.listdir(str(data_dir)+"//DATASET-Test-120//new_set_2//"+str(items)):
			#	for img in os.listdir(str(data_dir)+"//DATASET-Test-120//"+str(items)+"//"+str(addr_1)):
					#print("img",img)
				count +=1
				img_set.append(scipy.misc.imread(str(data_dir)+"//DATASET-Test-120//new_set_2//"+str(items)+"//"+str(img)))
			print(" the "+str(items)+" is "+str(count))
			random.seed(SEED)
			random.shuffle(img_set)
