...
class EDSR(object):
	...
	def __init__(self,img_size=32,num_layers=32,feature_size=256,scale=2,output_channels=1):
		...
		for i in range(num_layers):
			x = utils.resxBlock(x,channels=feature_size,scale=scaling_factor)
	...
	def train(self,decay_steps=1000,decay_rate=0.9,iterations=4000,save_dir="saved_models"):
		...
		with self.sess as sess:
			...
			for i in range(iterations):
				...
				x,y,batch_index = self.data(*self.args) #execute the get_batch function each iteration(almost 20 batch in one epoch)#argument with * is iterable
