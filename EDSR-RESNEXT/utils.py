...
'''
apply aggreated resnet transformation

'''
def resxBlock(x,channels=64,cardinality=16,kernel_size=[3,3],scale=1):
	layer_split = []
	for i in range(cardinality):
		split=slim.conv2d(x,channels/cardinality,kernel_size=[3,3],activation_fn=None)
		split=tf.nn.relu(split)
		split=slim.conv2d(split,channels/cardinality,kernel_size=[3,3],activation_fn=None)
		layer_split.append(split)
	return x+tf.concat(layer_split,axis=3)*scale
