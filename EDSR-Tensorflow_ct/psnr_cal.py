...
def psnr(output_img,input_img,target_img,shrunk_size=60,save_dir="saved_models"):
    ...
    for index in range(input_img.shape[0]):
		...
		cv.imwrite(save_dir+"image//input//"+str(index+2)+".png",input)
        output=output_img[index,:,:,:].reshape(output_img.shape[1],output_img.shape[1])
		cv.imwrite(save_dir+"image//output//"+str(index+2)+".png",output)
        target=target_img[index,:,:,:].reshape(target_img.shape[1],target_img.shape[1])
		cv.imwrite(save_dir+"image//target//"+str(index+2)+".png",target)
